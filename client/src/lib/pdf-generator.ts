...
export function generateInvoicePDF(invoice: InvoiceWithDetails): void {
  ...
  if (invoice.payments.length > 0) {
    ...
    invoice.payments.forEach((payment, index) => {
      ...
      doc.text(`${formatDate(payment.paymentDate!)} - ${payment.paymentMethod}: ${formatCurrency(payment.amount)}`, 20, y);
    });
  }
  ...
  doc.text('For any queries, please contact our billing department.', 20, pageHeight - 20);
